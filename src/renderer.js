...
const baseTempDirs = p.resolve(__dirname, 'templates')
...
class Renderer {
  ...
  constructor(Fs) {
    this.Fs  = Fs
    this.env = nunjucks.configure([this.Fs.tempDir, baseTempDirs], {noCache : true});
    this.md  = new MarkdownIt(mdConfig);
    this.addGlobal("baseDefaultTemplate", p.resolve(baseTempDirs, 'base.html'))
  ...
  render(template, data = {}) {
    data = this.parseVersion(template, data)
  ...
  parseVersion(template, data) {
    let version = p.parse(template).dir.split(this.Fs.config.docsDir)[1].substr(1).split('\\')[0]
