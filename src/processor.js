...
class Processor {
  ...
  buildFile(file, path = null) {
    if (path) {
      file = path.split(this.Fs.docsDir)[1].substr(1);
    }
    const srcPath = p.resolve(this.Fs.docsDir, file)
  ...
  watch() {
    ...
    chokidar.watch([this.Fs.tempDir, this.Fs.staticDir], watcherConfig)
      .on('all', (event, name) => {
        if (event === 'add' || event === 'change' || event === 'unlink') {
          console.log(`Changes found in ${name}`)
          if (!name.includes(this.Fs.docsDir)) {
            this.cleanBuildDir();
            this.genrateBuildDir()
          } else {
            const file = name.split(this.Fs.docsDir)[1].substr(1);
            this.buildFile(file)
          }
        }
      }).on('error', (...e) => {
